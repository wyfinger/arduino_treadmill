...
#define pinOut      5  // D5
#define pinRelay    4  // D4
#define pinAccel   A0  // нога на которой висит требуемая нам нога акселерометра
...
long AngleLastTime = 0;  // время, когда последний раз проверялся угол наклона полотна
...
void SendPacket(char Code, int Value) 
{
  ...
  buff[0] = 'P';          // это означает, что пакет от силового модуля на дисплей
  buff[1] = Code;
...
void CheckAngle()
{
  const long UpdatePeriod  = 2000 * FreqFactor;   // слишком часто проверять нет смылса, чтобы не забивать канал связи с дисплеем ерундой
  const int dx = 296;
  const int dy = 341;
  const int dz = 350;

  if (millis() - AngleLastTime > UpdatePeriod)
  {
    // считываем данные с акселерометра
    int Val = map(analogRead(pinAccel), 0, 1023, 0, 90); // TODO: здесь подправить потом!
    SendPacket('A', Val);
    AngleLastTime = millis();  // чтобы часто не слать
  }
