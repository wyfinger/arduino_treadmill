#line 1 "C:/Treadmill/Arduino/SendPacketTest/two/two.ino"
#include <Arduino.h>
...
void SendPacket(char Code, int Value)
{
  ...
  buff[0] = 'T';          // это означает, что пакет от силового модуля на дисплей
  buff[1] = Code;
...
void RecivePacket()
{
  if (Serial.available() > 0)
  {
    ...
    } else // закончилась строка, возможно это валидный пакет
    {
      ...
      if ((Direction == 'O') && (Crc == RealCrc))  // команду в работу !
      {
        switch (Code)
        {
