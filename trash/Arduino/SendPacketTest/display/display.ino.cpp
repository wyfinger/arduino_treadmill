#line 1 "C:/Treadmill/All/display/display.ino"
/*
...
int   CurrAngle = 15;       // угол наклона полотна, в градусах
int   CurrTemperature = 25; // температура радиатора силового модуля
...
long PulseLastTime;    // время последнего удара сердца
...
void RecivePacket()
{
  if (Serial.available() > 0)
  {
    ...
    } else // закончилась строка, возможно это валидный пакет
    {
      ...
      int Value = 1000*(ReciveBuff[2]-0x30) + 100*(ReciveBuff[3]-0x30) + 10*(ReciveBuff[4]-0x30) + 1*(ReciveBuff[5]-0x30);

      // вот это задротство ниже чтобы конвертировать одит байт HEX -> Byte
      String hexCrc = "0x"+ReciveBuff[6]+ReciveBuff[7];
...
void BeepLong()
{
  /// TODO: Либо пикнуть дольше, либо сделать 2-3 писка - ДОПИСАТЬ
  Beep();
}
  ...
  if (millis() - PulseStamp > 2000)
  {
    ...
    for (int i = 0; i<sizeof(PulseAveraging); i++)
    {
      if (PulseAveraging[i] > 0)
      {
        ...
        Dbg = Dbg + " " + String(PulseAveraging[i]);
        PulseCount++;
      }
    }
    Serial.println(Dbg);
